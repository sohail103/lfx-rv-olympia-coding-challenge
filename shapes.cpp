...
bool Square::parseAndCalculate(const std::vector<std::string>& args) {
    if(args.size() == 0) {
        std::cout << "A square has four sides that are of equal length. If the user provides the length:\n"
...
std::string Rectangle::getDescription() const {
    return "A rectnagle has 4 sides broken into 2 side pairs of equal length and are parallel.";
}
...
bool Rectangle::parseAndCalculate(const std::vector<std::string>& args) {
    if (args.size() == 0) {
        ...
        std::cerr << "Error: Rectangle requires exactly 2 arguments (length1, length2)\n";
        return false;
    }
    else {
        try {
